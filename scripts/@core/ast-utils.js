...
function insertImport(source, fileToEdit, symbolName, fileName, isDefault) {
    ...
    var useStrict = findNodes(rootNode, ts.SyntaxKind.StringLiteral)
...
function insertAfterLastOccurrence(nodes, toInsert, file, fallbackPos, syntaxKind) {
    ...
    var lastItemPosition = lastItem ? lastItem.getEnd() : fallbackPos;
...
function getFirstNgModuleName(source) {
    ...
    var ngModulesMetadata = getDecoratorMetadata(source, 'NgModule', '@angular/core');
    if (ngModulesMetadata.length === 0) {
        return undefined;
    }
    // Then walk parent pointers up the AST, looking for the ClassDeclaration parent of the NgModule
    // metadata.
    var moduleClass = findClassDeclarationParent(ngModulesMetadata[0]);
