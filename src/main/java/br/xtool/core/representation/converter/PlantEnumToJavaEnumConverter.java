...
public class PlantEnumToJavaEnumConverter implements BiFunction<SpringBootProjectRepresentation, PlantEnumRepresentation, JavaEnumRepresentation> {
	...
	@Override
	public JavaEnumRepresentation apply(SpringBootProjectRepresentation bootProject, PlantEnumRepresentation plantEnum) {
		// @formatter:off
		JavaEnumRepresentation javaEnum = bootProject.getRoasterJavaUnits().stream()
				...
				.findFirst()
				.orElseGet(() -> new EJavaEnumImpl(bootProject,RoasterUtil.createJavaEnumSource(plantEnum.getUmlPackage().getName(),plantEnum.getName())));
